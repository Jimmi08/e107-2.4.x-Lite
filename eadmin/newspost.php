...
class news_admin_ui extends e_admin_ui
{
	...
	public function afterCreate($new_data, $old_data, $id)
	{
		...
		{
			// LITE MODIFICATION (#84): inject news_id so notify's e107::url('news','item',$data) has news_id + news_sef and the SEF/legacy URL resolves
			$new_data['news_id'] = $id;
		...
		{		
			if(!isset($_POST['submit_news']))
			{
				...
				//		$this->noPermissions();
					...
					$_POST['comment_total'] = e107::getDb()->createQueryBuilder()->from('comments')->where('comment_item_id', (int) $row['news_id'])->where('comment_type', '0')->count();
